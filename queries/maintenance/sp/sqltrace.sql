/*---------------------------------------------------------------------
  $Header: /WWW/sqlutil/sqltrace.sp 5     12-10-27 22:34 Sommar $
   ...
   ADD EVENT sqlos.wait_info
    (ACTION (package0.collect_system_time)
     WHERE sqlserver.session_id=' + CAST(@spid as varchar) +
           ' AND opcode=1 AND wait_type<>387)
...
do_analysis:
   ...
   IF @show_waits = 1 BEGIN
      ...
      WITH extracted_data AS (
         SELECT   rowno,
                  ...
                  d.data.value('(text/text())[1]', 'nvarchar(100)') AS text_elem,
                  d.data.value('(value/text())[1]', 'nvarchar(100)') AS value_elem
         FROM #xe_file x
      ...
      SELECT agg.id, coalesce(SP.csum, 0), agg.wait_type, agg.duration, agg.signal_duration,
      ...
      FROM (
         ...
         FROM (
            SELECT rowno,
               MIN (CASE what WHEN 'wait_type' THEN text_elem END) wait_type,
   ...
   IF @snooping = 1 BEGIN
      ...
      VALUES (getdate(), 41, 'EXEC (snoop)', 'sp_sqltrace',
               0, -1, @snoop_time*1000000, 9223372036854775807);
   END
